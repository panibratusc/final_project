...
def get_author_and_date_NYT(url_series):
    ...
    for url in url_series:
        page = requests.get(url)
        soup = BeautifulSoup(page.content, 'html.parser')
        article_date = soup.select('div[class_ = "css-3xqm5e"] time')[0].contents[0]
        article_author = soup.select('span.last-byline')[0].text
        result.append((article_author, article_date))
    return result
...
def get_NYT_IsraelPalestine(limit=None):
    ...
    if response.status_code == 200:
        ...
        headlines_NYTIsraelPalestine = [article.find('h2', class_='css-1l8buln') for article in article_wrappers]
